...
def configure_existing(sid: str, route: str, public_base: str) -> dict:
    ...
    client = _client()
    num = client.phone_numbers.get(sid)
    e164 = num.get("number")
    assignment = assign_number_to_agent(e164, public_base=public_base, route=route, client=client)
    setup = {
        "sid": num.get("id"),
        "phone_number": e164,
        "friendly_name": num.get("name") or "",
...
def purchase_and_configure(phone_number: str, route: str, public_base: str) -> dict:
    ...
    client = _client()
    bought = client.phone_numbers.create(number=phone_number)
...
def repoint_to_route(route: str, public_base: str) -> dict:
    ...
    assignment = assign_number_to_agent(phone_number, public_base=public_base, route=route)
